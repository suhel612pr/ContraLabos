...
3. `storage.sql` — creates the three file-storage buckets and their access
   rules
4. `marketplace-migration.sql` — adds jobs, worker applications,
   transactional hiring, and the hired-workers list

Run the marketplace migration after the first three files. It is additive and
does not change existing project, worker, or payment data.
